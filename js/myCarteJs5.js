...
$(document).ready(function(){
	...
	function appelerMenu_paquet(){
		...
		if (action != null){
			switch(action){
				...
				break;
				case "P":
				    console.log("PIOCHE:_____________"+paquet2.cartes.length);
					
					//piocher une carte la première
					pioche = paquet2.cartes.shift();
					carte_piochee=JSON.parse(pioche);
				    
					console.log("CARTE PIOCHEE="+carte_piochee.nom);
					...
					console.log(">> CIMETIERE="+cimetiere);
					
					idxDispo = trouverIndiceDispo(me);
					console.log(">> idxdispo="+idxDispo);
    ...
    function rechercherCarteDsAtt(id){
		console.log("fn rechercherCarteDsAtt(): id="+id);
		if (att_me[0].id == id){
			carte = att_me[0];
		}else{
			carte = att_me[1];
	...
	function poserCarteEnJeu(carte, me,ctr){
		console.log("poserCarteEnJeu(): "+carte.nom);
	...
	function trouverIndiceDispoAdv(){
	    i=0;
		c04existe =(typeof $('#carte04') == 'undefined'); 
		console.log("ADV_C04existe? "+c04existe);
		if (!c04existe){  //la carte n'existe pas 
			i=3;
		}else{	
			trouve = false;
	...
	function trouverIndiceDispoMe(){
		i=0;
		c04existe =(typeof $('#mcarte04') == 'undefined'); 
		console.log("ME_C04existe? "+c04existe);
		if (!c04existe){  //la carte n'existe pas 
			i=3;
		}else{			
			trouve = false;
	...
	/*OK fontion de mélange d'un tableau
