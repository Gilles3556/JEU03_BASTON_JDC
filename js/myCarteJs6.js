...
$(document).ready(function(){
	...
	while(!finJeu){
		//DEBUT IA
		//SI Carte en ATT* => choisi entre CA, C1 ou C2 (si carte adversaire en attaque) OUX CC1,CC2,CC3,CC4 
		IA_jouer();
		
		ME_jouer();
				
	    finJeu=(pv_adv==0 || pv_me==0);	
	}
	/* --------------------------
	   ME joue
	   ------------------------- */
	...
	/* --------------------------
	 ...
	 function IA_placerEnAttaque(){
		 ...
		 console.log("IA_placerEnAttaque(): idx="+idx);
		 if (idx>-1){
			// si oui , placer an ATT_ADV1 ou ATT_ADV2
		 	cartePlacable = jeu_adv[idx];
			afficherDetailsCarte(cartePlacable);
			//choisir ATT_ADV1 ou ATT_ADV2
			noAtt= Math.trunc((Math.random()*0.5)+1);
			
			console.log("IA_placerEnAttaque(): "+cartePlacable.nom+" en "+noAtt);
			
			$("#"+cartePlacable.id).html("");
			strhtml=buildDivAttCarte(cartePlacable, false,noAtt);
			$(strhtml).appendTo("body");
			strhtml="";
		
			// placer carte
			cartePlacable.etat = (noAtt==1?ETAT_INV_ATT1:ETAT_INV_ATT2);
			
			cartePlacable.id="advAtt+noatt";
			att_adv[noAtt-1] = cartePlacable;
			
			placerCarteEnAttaque(false,cartePlacable.id,noAtt);
			//retirer des cartes en jeu
			jeu_adv[idx]='undefined';
			afficherJeux();
		 }
	 }
				...
				case "A2":
					if (cim.length<c.cout){
						...
						if (att_me[noAtt-1]== 0){
							...
							c.etat = (noAtt==1?ETAT_INV_ATT1:ETAT_INV_ATT2);
							c.id="meAtt"+noatt;
							att_me[noAtt-1] = c;			
							
							strhtml=buildDivAttCarte(c, true,noAtt);
							$(strhtml).appendTo("body");
							strhtml="";
							
							placerCarteEnAttaque(true,id,noAtt);
							//retirer des cartes en jeu
							jeu_me[no]='undefined';
						}else{
			...
			switch(action){
				case "F":
					if (att_me.length>0){
						...
						for(c=0;c<2;c++){
							if (att_me[c]!=='undefined'){
								if (att_me[c].etat == ETAT_INV_ATT1){
									att_me[c].etat = ETAT_ATT1;
								}else{
									if (att_me[c].etat = ETAT_INV_ATT2){
										att_me[c].etat = ETAT_ATT2;		
									}	
								}
							}
						}
	...
	/* OK------------------------------------------------
	   ...
	   en A1 ou A2
	--------------------------------------------------- */
	function placerCarteEnAttaque(me,idcarte,noatt){
		if(me){
			...
			$(idcarte).css("top","200px");
			$(idcarte).css("left",l);
			$(idcarte).addClass("attaque");
			console.log("placerCarteEnAttaque() ok")
		}
	}
	...
	*/
	...
	/* -----------------------------
	 Piocher une carte dans le jeu
	 -------------------------------- */
	function jeu_piocher(){
		...
		if (!cimetiere && idxDispo!=-1){						
			...
			console.log((me?PROMPT_ME:PROMPT_ADV)+" "+carte_piochee.nom+" mise en jeu pour "+(me?"me":"adv")+ " en "+idxDispo);
			...
			alert((me?PROMPT_ME:PROMPT_ADV)+" "+carte_piochee.nom+" placée ds cimetière !");
		}
	 }
	/* -----------------------------
	...
	function sacrifierUneCarte(no,laCarte, me, effacer){
		...
		console.log((me?PROMPT_ME:PROMPT_ADV)+" "+laCarte.nom+" est sacrifiée");
		majCompteursPaquet();
		afficherJeux();						 
	}
	/* ------------------------------
   ...
   /*OLD : idée 1
	function IA_jouer(){
		//ATT:CA ou autre
		hasard=Math.trunc(Math.random()*10);
		if (document.getElementById("carte01")!=null || document.getElementById("carte02")!=null ){ 
			//IA a des cartes en attaque
			if (hasard<=51){
				// Attaque CA
				c_att= null;
				//choisir carte qui attaque
				if (hasard<26 && document.getElementById("carte01")!=null ){ //ADV.ATT1
					c_att = att_adv[0];
					cible= att_me[Math.random()*0.5];
					
				} else {//ADV.ATT2
					if(document.getElementById("carte02")!=null ){
						c_att = att_adv[1];
						cible= att_me[Math.random()*0.5];
					}	
				}
				...
				if (hasard>25){
					//ATT c1 ou C2
					noCible = Math.random()*0.5; //0 ou 1 ???
					
				}else{
					//ATT CC1,CC2,CC3,CC4
					nocible = Math.random(4)+1; //1 à 4 ???
				}
				
			}
		}else{
			//Autres choix: S P ATT1/ATT2
